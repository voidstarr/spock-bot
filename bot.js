...
client.on("ready", () => {
  ...
  for (const g of client.guilds.array()) {
    for (const m of g.members.array()) {
      logUser(m);
    }
  }
});
...
client.on("message", msg => {
  ...
  if (msg.mentions.users.exists("id", client.user.id)) {
    var removedMention = msg.content
    ...
    msg.reply(res);
    logMessage(msg);
  } else if (msg.author.id == adminUserId && args[0] == "~!die") {
      ...
      db("channel_messages")
        ...
        .then(resp => {
          ...
          msg.channel.send(
            "I have **" +
              resp[0].a +
              "** messages from this server stored in the ship's memory banks."
          );
        })
        .catch(err => {
          logger.err(err);
    ...
    .then(rows => {
      if (rows.length === 0) {
        db("users")
          ...
          .then(rows => {
            console.log(`added new user: ${user.id}`);
          });
      }
    });
}
...
function logMessage(msg) {
  ...
  if (msg.channel instanceof Discord.TextChannel) {
    ...
    msg.channel instanceof Discord.GroupDMChannel
  ) {
    ...
    logger.debug(
      "something went wrong. could not determine type of message channel"
    );
  }
}
